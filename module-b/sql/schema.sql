...
CREATE UNIQUE INDEX `fee_payment_transaction_id_unique` ON `fee_payment` (`transaction_id`);
...
CREATE UNIQUE INDEX `hostel_block_block_name_unique` ON `hostel_block` (`block_name`);
...
CREATE UNIQUE INDEX `student_enrollment_no_unique` ON `student` (`enrollment_no`);
CREATE TABLE `users` (
